...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-14 rounded-lg bg-card text-foreground border px-2 ${className}`}>
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-10 h-[5px] rounded-full bg-primary shadow-[0_50px_15px_var(--primary)] ${
          isReady ? 'transition-[left] duration-400 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-30%] top-[5px] w-[160%] h-12 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-primary/30 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};
